...
def _new_page(browser, timeout: int = 30000):
...
def scrape_oddsportal(home_team: str, away_team: str) -> Optional[dict]:
    ...
    try:
        with sync_playwright() as p:
            ...
            # Keressük a meccset a találatok között
            ...
            # Ha nem találtuk a keresőben, próbáljuk a football szekciót
            page.goto("https://www.oddsportal.com/football/", wait_until="domcontentloaded")
            page.wait_for_timeout(3000)
...
def scrape_bettingexpert(home_team: str, away_team: str) -> Optional[dict]:
    ...
    try:
        with sync_playwright() as p:
            ...
            # Scroll le hogy betöltődjön a lazy content
            ...
            # Próbáljuk megtalálni az odds-ot a DOM-ban
...
def scrape_flashscore(home_team: str, away_team: str) -> Optional[dict]:
    ...
    try:
        with sync_playwright() as p:
            ...
            # Megnyitjuk a meccs oldalát és keressük az odds-ot
...
# ---------------------------------------------------------------------------
# FŐ FÜGGVÉNY — összesített odds gyűjtés (lecseréli az odds_scraper.py-t)
# ---------------------------------------------------------------------------

def get_best_odds_playwright(home_team: str, away_team: str) -> dict:
    """
    Összesített odds gyűjtés Playwright-tal.
    Lecseréli a get_best_odds() függvényt az odds_scraper.py-ban.
    """
    import statistics
